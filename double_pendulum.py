...
class DoublePendulum(ODEModel):
    def __init__(self, L1: float = 1, L2: float = 1, M1: float = 1, M2: float = 1, g: float = 9.81) -> None: # are the default arguments correct?
        """
    ...
    def __call__(self, t: float, u: np.ndarray):
        ...
        domega2_dt = (-self.L1*(omega1**2)*np.sin(dtheta)*np.cos(dtheta) + 2*self.g*np.sin(theta1)*np.cos(dtheta) - 2*self.L1*(omega1**2)*np.sin(dtheta) - 2*self.g*np.sin(theta2))/(2*self.L2 - self.L2*(np.cos(dtheta))**2)
...
class DoublePendulumResults:
    ...
    def potential_energy(self) -> np.ndarray:
        ...
        P2 = self.pendulum.M2*self.pendulum.g*(self.y1 + self.pendulum.L1 + self.pendulum.L2)
